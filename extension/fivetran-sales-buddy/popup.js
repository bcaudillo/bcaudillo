...
const glossaryData = [
  ...
  // Data Ecosystem
  { term: 'ODI', simple: 'Oracle Data Integrator — a legacy ETL tool customers are often migrating away from', detailed: 'Oracle Data Integrator is an on-premises ETL (Extract, Transform, Load) platform by Oracle. Common in large enterprises running Oracle databases. It requires dedicated infrastructure, specialized developers, and significant maintenance. Unlike Fivetran, ODI transforms data before loading it (ETL, not ELT), meaning raw data is not preserved in the destination.', whyMatters: 'Many enterprise prospects mention ODI as their current tool. It\'s a direct displacement opportunity — Fivetran offers managed, cloud-native ELT with no infrastructure, vs ODI which requires DBAs, Java developers, and servers to maintain.', example: 'Enterprise customer says "we use ODI to move data from Oracle to Redshift." That\'s a Fivetran displacement opportunity — they\'re using a 2010s tool for a problem Fivetran solves fully managed in the cloud.', category: 'Competitive' },
  { term: 'ETL', simple: 'Extract, Transform, Load — the legacy approach Fivetran replaced', detailed: 'The traditional data integration pattern where data is extracted from source, transformed (cleaned, filtered, aggregated) before loading, then loaded into the destination. ETL tools include Informatica, Talend, ODI, SSIS, and Pentaho. Unlike ELT, raw data is lost — only the pre-transformed version lands in the warehouse.', whyMatters: 'When customers say "we do ETL" or name a legacy tool, that\'s a migration opportunity. Fivetran\'s ELT approach is superior: raw data is preserved, transformations are flexible, and no infrastructure is needed.', example: 'Customer uses Informatica to filter and aggregate Salesforce data before loading to Teradata. Migrating to Fivetran means raw data in Snowflake, with dbt handling transformations — far more flexible.', category: 'Competitive' },
